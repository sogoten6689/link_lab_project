class Admin::MyLabsController < ApplicationController
  include AdminHelper
  layout "admin"
  ...
  def index
    ...
    @breadcrumbs = [[t('labs.name'), admin_labs_url]]
  end

  def new
    @lab = Lab.new
    @breadcrumbs = [[t('labs.name'), admin_labs_url], [t('labs.new'), new_admin_lab_url]]
  end
  ...
  def edit
    ...
    @breadcrumbs = [[t('labs.name'), admin_labs_url], [t('labs.edit'), edit_admin_lab_url(params[:id])]]
  end
  ...
  private
    ...
    def invalid_foreign_key
      redirect_to admin_labs_path, {alert: t('labs.errors.invalid_foreign_key')}
    end
end

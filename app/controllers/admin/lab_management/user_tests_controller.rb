class Admin::LabManagement::UserTestsController < ApplicationController
  ...
  def index
    ...
    @breadcrumbs = [[t('user_tests.name'), admin_lab_management_user_tests_url]]
  end
  ...
  def new
    ...
    @breadcrumbs = [[t('user_tests.name'), admin_lab_management_user_tests_url], [t('user_tests.new_user_tests'), new_admin_lab_management_user_test_url]]
  end
  ...
  def create
    ...
    if @user_test.save
      ...
      redirect_to  admin_lab_management_user_tests_path
    else
  ...
  def show
    ...
    @lab_tests = RawTest.joins(:test_type, :lab_test).select('lab_tests.id, concat(raw_tests.name,  \' - \', test_types.vi_name) as name');
    @breadcrumbs = [[t('user_tests.name'), admin_data_management_labs_url], [t('user_tests.name'), edit_admin_data_management_lab_url(params[:id])]]
  end
  ...
  def edit
    ...
    @user_test.lab_test_ids = @result_test_ids
    @lab_tests = RawTest.joins(:test_type, :lab_test).select('lab_tests.id, concat(raw_tests.name,  \' - \', test_types.vi_name) as name');
    @breadcrumbs = [[t('user_tests.name'), admin_lab_management_user_test_url], [t('user_tests.edit_lab_test'), edit_admin_user_url(params[:id])]]
  end
  ...
  def update
    ...
    if @user_test.update(eidt_user_params)
      if @user_test.test_status == 0
        ...
        redirect_to  admin_lab_management_user_test_path(@user_test.id)
      end
    else
      @doctors = User.all.where(:role => 1)
      @users = User.all.where(:role => 2)
  ...
  private
    ...
    def invalid_foreign_key
      redirect_to admin_lab_management_user_test_path, {alert: t('user_test.errors.invalid_foreign_key')}
    end
end
